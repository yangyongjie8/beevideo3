...
import com.skyworthdigital.voice.videosearch.callback.MetroItemFocusListener;
import com.skyworthdigital.voice.videosearch.callback.MetroItemLongClickListener;
import com.skyworthdigital.voice.videosearch.callback.OnMoveToListener;
...
public class MetroRecyclerView extends RecyclerView {
     ...
     * 滚动允许的误差值
     */
    private final static int DEVIATION = 0;
    /**
     * 在最后一行才往下滚
     */
    ...
    public interface OnScrollEndListener {
        void onScrollToBottom(int keyCode);

        void onScrollToTop(int keyCode);
    }
    ...
    /*protected final class Status {
        ...
        public void initVisibleItemPosition(LayoutManager layoutManager) {
            ...
            lastCompletelyVisibleItemPosition =
                    findLastCompletelyVisibleItemPosition(firstVisibleItemPosition, lastVisibleItemPosition);
            // 屏幕中能显示到的view(有时候有些firstVisibleItemPosition不在屏幕内)
            firstVisibleItemPositionInScreen =
                    findFirstVisibleItemPositionInScreen(firstVisibleItemPosition, firstCompletelyVisibleItemPosition);
            lastVisibleItemPositionInScreen =
                    findLastVisibleItemPositionInScreen(
            ...
            if (DEBUG) {
                ...
                Log.i(TAG, "Status lastVisibleItemPositionInScreen:" + lastVisibleItemPositionInScreen);
            }
        }
    ...
    public MetroRecyclerView(Context context, AttributeSet attrs, int defStyle) {
        ...
        if (attrs != null) {
            ...
            boolean isFocusViewOnFrontEnable =
                    a.getBoolean(R.styleable.MetroRecyclerView_focusViewOnFrontEnable, false);
            ...
            SCALE = a.getFloat(R.styleable.MetroRecyclerView_scale, 1.1f);
    ...
    /*public void setNextFocusUpView(View... nextFocusUpView) {
        this.mNextFocusUpView = nextFocusUpView;
    }*/
    ...
    /*private boolean focusToTop() {
        if (mNextFocusUpView != null && mNextFocusUpView.length > 0) {
            for (View view : mNextFocusUpView) {
    ...
    /*private void upEvent() {
        int nextUpPosition = getSelectedPosition() - mNumColumns;
        if (nextUpPosition < 0) {
            focusToTop();
        }
    }*/
    ...
    private void initSmoothScrollMethod() {
        try {
            ...
            mSmoothScrollMethod =
                    mViewFlinger.getClass().getDeclaredMethod(
                            "smoothScrollBy",
                            int.class,
                            int.class,
                            int.class,
    ...
    private void initScrollEvent() {
        addOnScrollListener(new OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                // 处理误差
                if (Math.abs(mLeftDistance) <= DEVIATION && mLeftDistance != 0) {
                    if (DEBUG) {
                        Log.i(TAG, "deviation process:" + mLeftDistance);
                    }
                    stopScroll();
                    scrollExtraForEnd();
                    return;
    ...
    private void bindView() {
        if (!isDelayBindEnable || mRecycler == null || mAttachedViewArray == null) {
            return;
        }
        isStartBind = true;
    ...
    /*protected void superOnFocusChanged(boolean gainFocus, int direction, Rect previouslyFocusedRect) {
        super.onFocusChanged(gainFocus, direction, previouslyFocusedRect);
    }*/
     ...
     *
     *
     * @see //SCROLL_TYPE_ON_LAST 在最后一行才往下滚
     * @see //SCROLL_TYPE_ALWAYS_LEFT_ONE 留一行表示出未到最后一行,底下留一行就开始滚
     * @see //SCROLL_TYPE_ALWAYS_CENTER
     * 像电视淘宝一样，把下一行放到RecyclerView的正中间,屏幕中的view必须最多三行
     */
    ...
    /*public boolean deleteItem(int position, List<?> data) {
        return deleteItem(null, position, data);
    }*/
     ...
     * @param position
     */
    public boolean deleteItem(View view, int position, List<?> data) {
    ...
    private void deleteView(int deletePosition) {
        //LogUtil.log("deleteView deletePosition:" + deletePosition);
        ...
        if (leaveView != null) {
            ...
            leaveView(leaveView, leavePosition);
        ...
        if (selectView != null) {
            ...
            mOutAnimator.start();
            selectView(selectView, mPosition);
        }
    ...
    /*public void setScrollDistance(int distance) {
        mScrollDistance = distance + getItemSpaceTop() + getItemSpaceBottom();
    }
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (mKeyListener != null) {
            if (mKeyListener.onKey(this, keyCode, event)) {
                return true;
            }
        }
        if (event.getAction() == KeyEvent.ACTION_DOWN) {
            switch (keyCode) {
                ...
                case KeyEvent.KEYCODE_DPAD_RIGHT:
                    ...
                    return false;//move(virtualKeyCode, keyCode);
                case KeyEvent.KEYCODE_DPAD_CENTER:
                case KeyEvent.KEYCODE_ENTER:
                    if (!isFlying
                            && !isDeleting
                            && mPosition < getAdapter().getItemCount()
                            && mOnItemClickListener != null) {
                        mOnItemClickListener
                                .onItemClick(this, getLayoutManager().findViewByPosition(mPosition), mPosition);
                    }
                    break;
            }
    ...
    /*protected boolean superDispatchKeyEvent(KeyEvent event) {
        return super.dispatchKeyEvent(event);
    }*/
     ...
     * @return
     */
    /*private boolean move(int virtualKeyCode, int keyCode) {
        if (DEBUG) {
            Log.i(TAG, "move mLeftDistance:" + mLeftDistance);
            Log.i(TAG, "isFlying:" + isFlying);
            Log.i(TAG, "mPosition:" + mPosition);
            Log.i(TAG, "virtualKeyCode:" + virtualKeyCode + " keyCode:" + keyCode);
        }
        ...
        if (status.virtualKeyCode == VIRTUAL_KEY_CODE_NEXT_ROW
            ...
            View nowView = status.getChildView(mPosition);
            if (nowView != null) {
                scaleOut(nowView, mPosition);
            }
        }
        return true;
    }*/
    ...
    /*private boolean processSpecialKeyEvent(Status status) {
        if (waitScrolling
            ...
            return false;
                ...
                && (status.virtualKeyCode == VIRTUAL_KEY_CODE_NEXT_ONE || status.virtualKeyCode == VIRTUAL_KEY_CODE_PRE_ONE)
                && status.getNextChildView() == null) {
    ...
    private void scrollToNotDisplayRow(int firstVisibleItemPosition, int nextPosition) {
        ...
        mLastViewTop = getViewTopLocation(currView) - offset;
        mLastViewBottom = getViewHeight(currView) + mLastViewTop;
        LogUtil.log("scrollToNotDisplayRow mLastViewTop:" + mLastViewTop + " mLastViewBottom:" + mLastViewBottom);
        isFlying = true;
        mFlyDirection = DIRECTION_DOWN;
        isStartBind = false;
    ...
    /*private int computeNextPosition(Status status) {
        ...
        // Log.i("Catch", "nextPosition:" + nextPosition);
        return nextPosition;
    }*/
    ...
    /*public int findLastVisibleItemPositionInScreen() {
        ...
        int lastCompletelyVisibleItemPosition =
                findLastCompletelyVisibleItemPosition(firstVisibleItemPosition, lastVisibleItemPosition);
        return findLastVisibleItemPositionInScreen(
                firstVisibleItemPosition,
                lastVisibleItemPosition,
                lastCompletelyVisibleItemPosition);
    }*/
    ...
    /*private int findFirstVisibleItemPositionInScreen(
            int firstVisibleItemPosition,
            int firstCompletelyVisibleItemPosition) {
        int firstVisibleItemPositionInScreen = firstVisibleItemPosition;
        while (getChildAt(firstVisibleItemPositionInScreen - firstVisibleItemPosition) != null
                && getViewBottomLocation(
    ...
    private int findLastVisibleItemPositionInScreen(
            int firstVisibleItemPosition,
            int lastVisibleItemPosition,
            int lastCompletelyVisibleItemPosition) {
        ...
        while (getChildAt(lastVisibleItemPositionInScreen - firstVisibleItemPosition) != null
                && getViewTopLocation(
                getChildAt(lastVisibleItemPositionInScreen - firstVisibleItemPosition)) >= getViewBottomLocation(this)
        ...
        if (child != null) {
            if (isInBottomRow(status.nextPosition)) {
                ...
                if (isNeedScrollToCenter()) {
                    ...
                    mLeftDistance = 0;
                    mLeftDistance +=
                            getViewTopLocation(child)
                                    + getViewHeight(child) / 2
                                    - (getViewTopLocation(this) + getViewHeight(this) / 2);
                } else {
                    mLeftDistance += getViewHeight(child) + getItemSpaceTop() + getItemSpaceBottom();
                }
    ...
    private boolean isNeedScrollToCenter() {
        return mScrollType == SCROLL_TYPE_ALWAYS_CENTER && getRowsCount(getChildCount()) <= 2;
    }
    ...
    /*private boolean isNeedScrollDown(Status status) {
        if (status.nextPosition > status.lastCompletelyVisibleItemPosition
                || (!isScrollOnLastEnable()
                && status.nextPosition <= status.lastCompletelyVisibleItemPosition
                && status.lastCompletelyVisibleItemPosition == status.lastVisibleItemPositionInScreen
                && (getRowNumber(status.nextPosition) == getRowNumber(
                status.lastVisibleItemPositionInScreen)))
                || (isNeedScrollToCenter() && getRowNumber(status.nextPosition) < getTotalRows(status) - 1)) {
            // if (status.getNextChildView() == null
            ) {
    ...
    protected void selectView(View view, int position) {
        if (DEBUG) {
            Log.i(TAG, "selectView:" + position);
        }
        view.setSelected(true);
    }

    private void scaleIn(View view, int position, boolean isClearSelected) {
        if (view == null) {
            return;
        }
        if (isClearSelected) {
            leaveView(view, position);
        }
    }

    /*private void scaleIn(View view, int position) {
        scaleIn(view, position, true);
    }*/

    protected void leaveView(View view, int position) {
    ...
    private class SpacesItemDecoration extends ItemDecoration {
        ...
        public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
            ...
            //LogUtil.log("getItemOffsets position:" + position + " count:" + count + " clunmCount:" + clunmCount + " mColumnCount:" + mColumnCount);
            if (mOrientation == VERTICAL) {
                if (position < clunmCount) {
                    outRect.top += paddingTop;
                }
                //LogUtil.log("getItemOffsets paddingTop:" + paddingTop + " outRect.top:" + outRect.top);
                ...
                //LogUtil.log("getItemOffsets paddingBottom:" + paddingBottom + " outRect.bottom:" + outRect.bottom + " getRowsCount:" + getRowsCount(count, clunmCount));
            } else {
                if (position < clunmCount) {
                    outRect.left += paddingLeft;
                }
    ...
    public static class MetroGridLayoutManager extends GridLayoutManager {
        ...
        /**
         * @param //是否打开全局测量 (用与自动测量recyclerview的高度或宽度) 全局测量效率较低
         */
        ...
        public void onMeasure(Recycler recycler, State state, int widthSpec, int heightSpec) {
            //LogUtil.log("MetroGridLayoutManager widthSpec:" + MeasureSpec.getSize(widthSpec) + " heightSpec:" + MeasureSpec.getSize(heightSpec) + " count:" + state.getItemCount());
                ...
                if (getOrientation() == RecyclerView.VERTICAL) {
                    ...
                    if (measuredWidth == 0) {
                        ...
                        if (view != null) {
                            ...
                            measuredWidth =
                                    (view.getMeasuredWidth() + getLeftDecorationWidth(view) + getRightDecorationWidth(view))
                                            * getSpanCount();
                        }
    ...
    public static abstract class MetroAdapter<VH extends MetroViewHolder> extends Adapter<VH> {
         ...
         *
         * @see android.support.v7.widget.RecyclerView.Adapter#//onBindViewHolder(android
    ...
    public static abstract class MetroViewHolder extends ViewHolder implements View.OnClickListener, View.OnLongClickListener {
        ...
        void setOnItemClickListner(MetroItemClickListener listener, View parentView) {
        ...
        void setOnItemLongClickListner(MetroItemLongClickListener listener, View parentView) {
        ...
        public void onClick(View v) {
            if (itemClickListener != null) {
                itemClickListener.onItemClick(parentView, v, getAdapterPosition());
            }
        }
        ...
        public boolean onLongClick(View v) {
            if (itemLongClickListener != null) {
                return itemLongClickListener.onItemLongClick(parentView, v, getAdapterPosition());
            } else {
                return false;
            }
        }
